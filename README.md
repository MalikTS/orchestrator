...
Пример ответа:
{
  ...
  "actual_replicas": 3,
  "replicas": [
    {"id":"replica-0","node_ip":"127.0.0.1","port":8000,"uptime":"..."},
    ...
  ],
...
💡 Примечания
Все процессы запускаются как обычные бинарники (без изоляции).
Агент автоматически регистрируется в контроллере и отправляет ping каждые 10 сек.
Порты для workload выделяются автоматически (8000, 8001, ...).
